...
int main(int argc, char **argv)
{
    ...
    if ((mutex_sem = semget(SEM_MUTEX_KEY, 1, 0660 | IPC_CREAT)) == -1)
    {
        perror("semget");
        exit(1);
    }
    ...
    if ((buffer_count_sem = semget(SEM_BUFFER_COUNT_KEY, 1, 0660 | IPC_CREAT)) == -1)
    {
        perror("semget");
        exit(1);
    }
    ...
    if ((spool_signal_sem = semget(SEM_SPOOL_SIGNAL_KEY, 1, 0660 | IPC_CREAT)) == -1)
    {
        perror("semget");
        exit(1);
    }
...
void *spooler(void *arg)
{
    ...
    while (1)
    {
        ...
        /* Since there is only one thread (spooler) using the buffer_print_index,
