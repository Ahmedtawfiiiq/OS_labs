...
void producer(int commodity_number, float mean, float standard_deviation, float sleep_interval_ms)
{
    ...
    float sleep_interval = sleep_interval_ms / 1000.0;
    ...
    while (true)
    {
        ...
        t = time(NULL);
        tm = *localtime(&t);
        fprintf(stderr, "[%d/%d/%d %d:%d:%6.3lf] %s: generating a new value %f\n", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, (float)tm.tm_sec, umap2[commodity_number].c_str(), item);
        ...
        t = time(NULL);
        tm = *localtime(&t);
        fprintf(stderr, "[%d/%d/%d %d:%d:%6.3lf] %s: trying to get mutex on shared buffer\n", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, (float)tm.tm_sec, umap2[commodity_number].c_str());
        ...
        t = time(NULL);
        tm = *localtime(&t);
        fprintf(stderr, "[%d/%d/%d %d:%d:%6.3lf] %s: placing %f on shared buffer\n", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, (float)tm.tm_sec, umap2[commodity_number].c_str(), item);
        ...
        t = time(NULL);
        tm = *localtime(&t);
        fprintf(stderr, "[%d/%d/%d %d:%d:%6.3lf] %s: sleeping for %d ms\n", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, (float)tm.tm_sec, umap2[commodity_number].c_str(), (int)sleep_interval * 1000);
...
int main(int argc, char *argv[])
{
    ...
    float sleep_interval_ms;
    sleep_interval_ms = atof(argv[4]);

    producer(commodity_number, mean, standard_deviation, sleep_interval_ms);
